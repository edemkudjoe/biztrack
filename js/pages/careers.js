...
async function doCPLogin(){
  ...
  try{
    ...
    const myApp=apps.find(a=>a.email===_cpUser.email&&a.offerSent);
    if(myApp){showCPOffer(myApp);}
    else{showJobsList();}
...
async function doCPSignup(){
  ...
  try {
    ...
    // Improved error parsing to catch Vercel HTML errors
    const text = await r.text(); 
    let d;
    try { d = JSON.parse(text); } catch(err) { throw new Error('Server returned an invalid response: ' + text.substring(0, 500)); }
    ...
    console.error(e);
    msg.innerHTML=`<div class="al al-r">System Error: ${e.message}</div>`;
  }
}
...
function showJobsList(){
  ...
  if (cpToken) {
    fetch(`${API}/data/applicants`, {
      headers: { 'Authorization': `Bearer ${cpToken}` }
    })
    .then(r => r.json())
    .then(data => { if (data.records) DB.s('applicants', data.records); })
    .catch(() => {});
  }
  ...
  fetch(`${API}/data/job_postings`)
    .then(r => r.json())
    .then(data => {
      if(data.records) DB.s('job_postings', data.records);
      renderJobsList();
    })
...
function submitCareersApplication(){
  ...
  const edu=document.getElementById('cp-edu').value;
  ...
  const newApp={name,email,phone:document.getElementById('cp-phone').value,position:_cpSelectedJob?.title||'General Application',dept:_cpSelectedJob?.dept||'',education:document.getElementById('cp-edu').options[document.getElementById('cp-edu').selectedIndex]?.text,eduScore:parseInt(edu)||50,experience:parseInt(exp)||0,expScore:parseInt(exp)||0,field:document.getElementById('cp-field').value,previousEmployers:document.getElementById('cp-emp').value,skills:document.getElementById('cp-skills').value,coverLetter:document.getElementById('cp-cover').value,date:today(),status:'pending',source:'careers_portal'};
  ...
  fetch(`${API}/data/applicants`,{
    ...
    if(r.record){
      ...
      const i=a2.findIndex(x=>x.email===newApp.email&&x.position===newApp.position);
      if(i>=0){a2[i].id=r.record.id;DB.s('applicants',a2);}
    }
  }).catch(()=>{});
...
async function respondCPOffer(decision){
  ...
  if(i>=0){
    ...
    if(apps[i].id){
      const cpToken=localStorage.getItem('cp_jwt');
      fetch(`${API}/data/applicants/${apps[i].id}`,{
        method:'PUT',
        headers:{'Content-Type':'application/json','Authorization':`Bearer ${cpToken}`},
        body:JSON.stringify({offerStatus:decision})
      }).catch(()=>{});
    }
  }
  ...
  if(i>=0){
    ...
    if(apps[i].id){
      ...
      fetch(`${API}/data/applicants/${apps[i].id}`,{
        method:'PUT',
        headers:{'Content-Type':'application/json','Authorization':`Bearer ${cpToken}`},
        body:JSON.stringify({offerStatus:'negotiated',negotiation:{rate,hours,benefits,note}})
      }).catch(()=>{});
    }
  }
