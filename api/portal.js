...
module.exports = async function (req, res) {
  ...
  try {
    ...
    if (!secret && (route === 'portal-signup' || route === 'portal-login')) {
    ...
    if (route === 'portal-signup') {
      ...
      const token = jwt.sign({ id: data.id, email: data.email, role: 'applicant' }, secret, { expiresIn: '8h' });
    ...
    if (route === 'portal-login') {
      ...
      const token = jwt.sign({ id: user.id, email: user.email, role: 'applicant' }, secret, { expiresIn: '8h' });
